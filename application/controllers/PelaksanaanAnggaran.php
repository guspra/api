		...
		if($method != 'POST'){
			...
			if($check_auth_client == true){
		        	...
		        	if($response['status'] == 200){
						...
						if (empty($params['uraian']) || empty($params['url_file']) || empty($params['tanggal_pelaksanaan']) || empty($params['id_dipa'])) {//ISI NAMA PARAMETER INPUT POST NYA
							$respStatus = 400;
		...
		if($method != 'PUT' || $this->uri->segment(3) == '' || is_numeric($this->uri->segment(3)) == FALSE){
			...
			if($check_auth_client == true){
				...
				if($response['status'] == 200){
					...
					if (empty($params['uraian']) || empty($params['url_file']) || empty($params['tanggal_pelaksanaan']) || empty($params['id_dipa'])) { //CEK PARAMETER INPUT NYA
						$respStatus = 400;
						$resp = array('status' => 400,'message' =>  'Input form masih salah, silahkan coba lagi');
					} else {
